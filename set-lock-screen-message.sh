...
# =============================================================================
# LOCK SCREEN MESSAGE MANAGER - FIXED VERSION
# =============================================================================
...
clear_lock_message() {
    ...
    # Try to clear from preboot volume if possible
    local preboot_path
    preboot_path=$(find /System/Volumes/Preboot -name "com.apple.loginwindow.plist" 2>/dev/null | head -1)
    
    if [[ -n "$preboot_path" && -f "$preboot_path" ]]; then
        echo "$(date): Attempting to clear preboot volume..." >> "$LOG_FILE"
...
if pgrep -f "lockscreen_coordinator.sh" | grep -v $$ >/dev/null 2>&1; then
    echo "$(date): Another instance of lockscreen_coordinator.sh is already running, exiting" >> "$LOG_FILE"
    exit 0
fi
